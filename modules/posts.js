...
export const Post = {
  ...
  getPostsByUserId: async (user_id) => {
    return await db.select("*").from("posts").where("user_id", user_id);
  },
  ...
  .leftJoin('likes', function() {
    this.on('posts.post_id', '=', 'likes.post_id')
      .andOn('likes.user_id', '=', db.raw('?', [currentUser_id])); 
  })
...
export const getPostsByUserId = Post.getPostsByUserId;
export const checkLikes = Post.checkLikes;
